...
/**
 * Table class for vw_excp_dupmobilenumber
 */
class vw_excp_dupmobilenumber extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vw_excp_dupmobilenumber';
		$this->TableName = 'vw_excp_dupmobilenumber';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vw_excp_dupmobilenumber`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// StationId
		$this->StationId = new DbField('vw_excp_dupmobilenumber', 'vw_excp_dupmobilenumber', 'x_StationId', 'StationId', '`StationId`', '`StationId`', 3, 11, -1, FALSE, '`StationId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// StationName
		$this->StationName = new DbField('vw_excp_dupmobilenumber', 'vw_excp_dupmobilenumber', 'x_StationName', 'StationName', '`StationName`', '`StationName`', 200, 30, -1, FALSE, '`StationName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->StationName->Sortable = TRUE; // Allow sort
		$this->fields['StationName'] = &$this->StationName;

		// MobileNumber
		$this->MobileNumber = new DbField('vw_excp_dupmobilenumber', 'vw_excp_dupmobilenumber', 'x_MobileNumber', 'MobileNumber', '`MobileNumber`', '`MobileNumber`', 200, 15, -1, FALSE, '`MobileNumber`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->MobileNumber->Sortable = TRUE; // Allow sort
		$this->fields['MobileNumber'] = &$this->MobileNumber;

		// SubDivisionId
		$this->SubDivisionId = new DbField('vw_excp_dupmobilenumber', 'vw_excp_dupmobilenumber', 'x_SubDivisionId', 'SubDivisionId', '`SubDivisionId`', '`SubDivisionId`', 3, 11, -1, FALSE, '`SubDivisionId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// StationSetup
		$this->StationSetup = new DbField('vw_excp_dupmobilenumber', 'vw_excp_dupmobilenumber', 'x_StationSetup', 'StationSetup', '`StationSetup`', '`StationSetup`', 200, 50, -1, FALSE, '`StationSetup`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`vw_excp_dupmobilenumber`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "vw_excp_dupmobilenumberlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "vw_excp_dupmobilenumberview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vw_excp_dupmobilenumberedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vw_excp_dupmobilenumberadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("vw_excp_dupmobilenumberview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vw_excp_dupmobilenumberview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "vw_excp_dupmobilenumberadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vw_excp_dupmobilenumberadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vw_excp_dupmobilenumberedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vw_excp_dupmobilenumberadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vw_excp_dupmobilenumberdelete.php", $this->getUrlParm());
	}
