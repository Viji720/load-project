...
/**
 * Table class for obs_ksndmc_rainfall
 */
class obs_ksndmc_rainfall extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'obs_ksndmc_rainfall';
		$this->TableName = 'obs_ksndmc_rainfall';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`obs_ksndmc_rainfall`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// Raingauge_id
		$this->Raingauge_id = new DbField('obs_ksndmc_rainfall', 'obs_ksndmc_rainfall', 'x_Raingauge_id', 'Raingauge_id', '`Raingauge_id`', '`Raingauge_id`', 3, 11, -1, FALSE, '`Raingauge_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// obs_datetime
		$this->obs_datetime = new DbField('obs_ksndmc_rainfall', 'obs_ksndmc_rainfall', 'x_obs_datetime', 'obs_datetime', '`obs_datetime`', CastDateFieldForLike("`obs_datetime`", 0, "DB"), 135, 19, 0, FALSE, '`obs_datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->obs_datetime->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['obs_datetime'] = &$this->obs_datetime;

		// rainfall
		$this->rainfall = new DbField('obs_ksndmc_rainfall', 'obs_ksndmc_rainfall', 'x_rainfall', 'rainfall', '`rainfall`', '`rainfall`', 4, 10, -1, FALSE, '`rainfall`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`obs_ksndmc_rainfall`";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if ($val == NULL)
			...
			$keyFilter = str_replace("@obs_datetime@", AdjustSql(UnFormatDateTime($val, 0), $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "obs_ksndmc_rainfalllist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "obs_ksndmc_rainfalledit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "obs_ksndmc_rainfalladd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("obs_ksndmc_rainfallview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("obs_ksndmc_rainfallview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "obs_ksndmc_rainfalladd.php?" . $this->getUrlParm($parm);
		else
			$url = "obs_ksndmc_rainfalladd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("obs_ksndmc_rainfalledit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("obs_ksndmc_rainfalladd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("obs_ksndmc_rainfalldelete.php", $this->getUrlParm());
	}
	...
	public function renderEditRow()
	{
		...
		$this->obs_datetime->EditValue = FormatDateTime($this->obs_datetime->CurrentValue, 8);
