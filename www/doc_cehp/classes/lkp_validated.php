...
/**
 * Table class for lkp_validated
 */
class lkp_validated extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'lkp_validated';
		$this->TableName = 'lkp_validated';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`lkp_validated`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// validated
		$this->validated = new DbField('lkp_validated', 'lkp_validated', 'x_validated', 'validated', '`validated`', '`validated`', 3, 11, -1, FALSE, '`validated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->validated->IsPrimaryKey = TRUE; // Primary key field
		$this->validated->Nullable = FALSE; // NOT NULL field
		$this->validated->Required = TRUE; // Required field
		...
		// validatedName
		$this->validatedName = new DbField('lkp_validated', 'lkp_validated', 'x_validatedName', 'validatedName', '`validatedName`', '`validatedName`', 200, 100, -1, FALSE, '`validatedName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->validatedName->Sortable = TRUE; // Allow sort
		$this->fields['validatedName'] = &$this->validatedName;

		// is_station_allowed
		$this->is_station_allowed = new DbField('lkp_validated', 'lkp_validated', 'x_is_station_allowed', 'is_station_allowed', '`is_station_allowed`', '`is_station_allowed`', 200, 1, -1, FALSE, '`is_station_allowed`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->is_station_allowed->Sortable = TRUE; // Allow sort
		switch ($CurrentLanguage) {
			case "en":
				$this->is_station_allowed->Lookup = new Lookup('is_station_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
			case "kn":
				$this->is_station_allowed->Lookup = new Lookup('is_station_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// is_subdiv_allowed
		$this->is_subdiv_allowed = new DbField('lkp_validated', 'lkp_validated', 'x_is_subdiv_allowed', 'is_subdiv_allowed', '`is_subdiv_allowed`', '`is_subdiv_allowed`', 200, 1, -1, FALSE, '`is_subdiv_allowed`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->is_subdiv_allowed->Sortable = TRUE; // Allow sort
		switch ($CurrentLanguage) {
			case "en":
				$this->is_subdiv_allowed->Lookup = new Lookup('is_subdiv_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
			case "kn":
				$this->is_subdiv_allowed->Lookup = new Lookup('is_subdiv_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// is_circle_allowed
		$this->is_circle_allowed = new DbField('lkp_validated', 'lkp_validated', 'x_is_circle_allowed', 'is_circle_allowed', '`is_circle_allowed`', '`is_circle_allowed`', 200, 1, -1, FALSE, '`is_circle_allowed`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->is_circle_allowed->Sortable = TRUE; // Allow sort
		switch ($CurrentLanguage) {
			case "en":
				$this->is_circle_allowed->Lookup = new Lookup('is_circle_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
			case "kn":
				$this->is_circle_allowed->Lookup = new Lookup('is_circle_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// is_WRDO_allowed
		$this->is_WRDO_allowed = new DbField('lkp_validated', 'lkp_validated', 'x_is_WRDO_allowed', 'is_WRDO_allowed', '`is_WRDO_allowed`', '`is_WRDO_allowed`', 200, 1, -1, FALSE, '`is_WRDO_allowed`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->is_WRDO_allowed->Sortable = TRUE; // Allow sort
		switch ($CurrentLanguage) {
			case "en":
				$this->is_WRDO_allowed->Lookup = new Lookup('is_WRDO_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
			case "kn":
				$this->is_WRDO_allowed->Lookup = new Lookup('is_WRDO_allowed', 'lkp_validated', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// record_count
		$this->record_count = new DbField('lkp_validated', 'lkp_validated', 'x_record_count', 'record_count', '`record_count`', '`record_count`', 3, 11, -1, FALSE, '`record_count`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`lkp_validated`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('validated', $rs))
				AddFilter($where, QuotedName('validated', $this->Dbid) . '=' . QuotedValue($rs['validated'], $this->validated->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->validatedName->DbValue = $row['validatedName'];
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('validated', $row) ? $row['validated'] : NULL;
		else
			$val = $this->validated->OldValue !== NULL ? $this->validated->OldValue : $this->validated->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@validated@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "lkp_validatededit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "lkp_validatedadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("lkp_validatedview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("lkp_validatedview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "lkp_validatedadd.php?" . $this->getUrlParm($parm);
		else
			$url = "lkp_validatedadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("lkp_validatededit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("lkp_validatedadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("lkp_validateddelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "validated:" . JsonEncode($this->validated->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->validated->CurrentValue != NULL) {
			$url .= "validated=" . urlencode($this->validated->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("validated") !== NULL)
				$arKeys[] = Param("validated");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->validated->CurrentValue = $key;
			else
				$this->validated->OldValue = $key;
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->validatedName->setDbValue($rs->fields('validatedName'));
	...
	public function renderEditRow()
	{
		...
		$this->validatedName->EditValue = $this->validatedName->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->validatedName);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->validatedName);
