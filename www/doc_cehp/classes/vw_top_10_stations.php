...
/**
 * Table class for vw_top_10_stations
 */
class vw_top_10_stations extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vw_top_10_stations';
		$this->TableName = 'vw_top_10_stations';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vw_top_10_stations`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// SMSDateTime
		$this->SMSDateTime = new DbField('vw_top_10_stations', 'vw_top_10_stations', 'x_SMSDateTime', 'SMSDateTime', '`SMSDateTime`', CastDateFieldForLike("`SMSDateTime`", 7, "DB"), 135, 19, 7, FALSE, '`SMSDateTime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// StationId
		$this->StationId = new DbField('vw_top_10_stations', 'vw_top_10_stations', 'x_StationId', 'StationId', '`StationId`', '`StationId`', 3, 11, -1, FALSE, '`StationId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// MobileNumber
		$this->MobileNumber = new DbField('vw_top_10_stations', 'vw_top_10_stations', 'x_MobileNumber', 'MobileNumber', '`MobileNumber`', '`MobileNumber`', 200, 25, -1, FALSE, '`MobileNumber`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->MobileNumber->Sortable = TRUE; // Allow sort
		$this->fields['MobileNumber'] = &$this->MobileNumber;

		// rainfall
		$this->rainfall = new DbField('vw_top_10_stations', 'vw_top_10_stations', 'x_rainfall', 'rainfall', '`rainfall`', '`rainfall`', 4, 7, -1, FALSE, '`rainfall`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// SMSText
		$this->SMSText = new DbField('vw_top_10_stations', 'vw_top_10_stations', 'x_SMSText', 'SMSText', '`SMSText`', '`SMSText`', 200, 50, -1, FALSE, '`SMSText`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`vw_top_10_stations`";
	}
	...
	{
		return ($this->SqlOrderBy != "") ? $this->SqlOrderBy : "`rainfall` DESC";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "vw_top_10_stationslist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "vw_top_10_stationsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vw_top_10_stationsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vw_top_10_stationsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("vw_top_10_stationsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vw_top_10_stationsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "vw_top_10_stationsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vw_top_10_stationsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vw_top_10_stationsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vw_top_10_stationsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vw_top_10_stationsdelete.php", $this->getUrlParm());
	}
