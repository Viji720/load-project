...
/**
 * Table class for vw_mst_station_mob
 */
class vw_mst_station_mob extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vw_mst_station_mob';
		$this->TableName = 'vw_mst_station_mob';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vw_mst_station_mob`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// StationId
		$this->StationId = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_StationId', 'StationId', '`StationId`', '`StationId`', 3, 11, -1, FALSE, '`StationId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// StationName
		$this->StationName = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_StationName', 'StationName', '`StationName`', '`StationName`', 200, 30, -1, FALSE, '`StationName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->StationName->Sortable = TRUE; // Allow sort
		$this->fields['StationName'] = &$this->StationName;

		// StationName_kn
		$this->StationName_kn = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_StationName_kn', 'StationName_kn', '`StationName_kn`', '`StationName_kn`', 200, 30, -1, FALSE, '`StationName_kn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->StationName_kn->Sortable = TRUE; // Allow sort
		$this->fields['StationName_kn'] = &$this->StationName_kn;

		// MobileNumber
		$this->MobileNumber = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_MobileNumber', 'MobileNumber', '`MobileNumber`', '`MobileNumber`', 200, 15, -1, FALSE, '`MobileNumber`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->MobileNumber->Sortable = TRUE; // Allow sort
		$this->fields['MobileNumber'] = &$this->MobileNumber;

		// SubDivisionId
		$this->SubDivisionId = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_SubDivisionId', 'SubDivisionId', '`SubDivisionId`', '`SubDivisionId`', 3, 11, -1, FALSE, '`SubDivisionId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// mst_station_owner
		$this->mst_station_owner = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_mst_station_owner', 'mst_station_owner', '`mst_station_owner`', '`mst_station_owner`', 3, 11, -1, FALSE, '`mst_station_owner`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->mst_station_owner->IsPrimaryKey = TRUE; // Primary key field
		$this->mst_station_owner->Nullable = FALSE; // NOT NULL field
		$this->mst_station_owner->Required = TRUE; // Required field
		$this->mst_station_owner->Sortable = TRUE; // Allow sort
		$this->mst_station_owner->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['mst_station_owner'] = &$this->mst_station_owner;

		// first_reading_date
		$this->first_reading_date = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_first_reading_date', 'first_reading_date', '`first_reading_date`', CastDateFieldForLike("`first_reading_date`", 0, "DB"), 133, 10, 0, FALSE, '`first_reading_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->first_reading_date->Sortable = TRUE; // Allow sort
		$this->first_reading_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['first_reading_date'] = &$this->first_reading_date;

		// last_reading_date
		$this->last_reading_date = new DbField('vw_mst_station_mob', 'vw_mst_station_mob', 'x_last_reading_date', 'last_reading_date', '`last_reading_date`', CastDateFieldForLike("`last_reading_date`", 0, "DB"), 133, 10, 0, FALSE, '`last_reading_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->last_reading_date->Sortable = TRUE; // Allow sort
		$this->last_reading_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`vw_mst_station_mob`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			...
			if (array_key_exists('mst_station_owner', $rs))
				AddFilter($where, QuotedName('mst_station_owner', $this->Dbid) . '=' . QuotedValue($rs['mst_station_owner'], $this->mst_station_owner->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	{
		return "`StationId` = @StationId@ AND `mst_station_owner` = @mst_station_owner@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('mst_station_owner', $row) ? $row['mst_station_owner'] : NULL;
		else
			$val = $this->mst_station_owner->OldValue !== NULL ? $this->mst_station_owner->OldValue : $this->mst_station_owner->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@mst_station_owner@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "vw_mst_station_moblist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "vw_mst_station_mobview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vw_mst_station_mobedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vw_mst_station_mobadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("vw_mst_station_mobview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vw_mst_station_mobview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "vw_mst_station_mobadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vw_mst_station_mobadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vw_mst_station_mobedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vw_mst_station_mobadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vw_mst_station_mobdelete.php", $this->getUrlParm());
	}
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",mst_station_owner:" . JsonEncode($this->mst_station_owner->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->mst_station_owner->CurrentValue != NULL) {
			$url .= "&mst_station_owner=" . urlencode($this->mst_station_owner->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("mst_station_owner") !== NULL)
				$arKey[] = Param("mst_station_owner");
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // StationId
					continue;
				if (!is_numeric($key[1])) // mst_station_owner
					continue;
				$ar[] = $key;
			}
		}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->StationId->CurrentValue = $key[0];
			else
				$this->StationId->OldValue = $key[0];
			if ($setCurrent)
				$this->mst_station_owner->CurrentValue = $key[1];
			else
				$this->mst_station_owner->OldValue = $key[1];
	...
	public function renderEditRow()
	{
		...
		if (!$Security->isAdmin() && $Security->isLoggedIn() && !$this->userIDAllow("info")) { // Non system admin
			$this->MobileNumber->CurrentValue = CurrentUserID();
		...
		$this->first_reading_date->EditValue = FormatDateTime($this->first_reading_date->CurrentValue, 8);

		// last_reading_date
		$this->last_reading_date->EditAttrs["class"] = "form-control";
		$this->last_reading_date->EditCustomAttributes = "";
		$this->last_reading_date->EditValue = FormatDateTime($this->last_reading_date->CurrentValue, 8);
	...
	public function addUserIDFilter($filter = "", $id = "")
	{
		...
		if (!$this->userIDAllow($id) && !$Security->isAdmin()) {
			...
			if ($filterWrk != "")
				$filterWrk = '`MobileNumber` IN (' . $filterWrk . ')';
		}
	...
	public function getUserIDSubquery(&$fld, &$masterfld)
	{
		global $UserTable;
		$wrk = "";
		$sql = "SELECT " . $masterfld->Expression . " FROM `vw_mst_station_mob`";
