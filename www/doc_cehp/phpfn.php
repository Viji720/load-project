...
class AdvancedSecurity
{
	...
	public function setupUserLevelEx()
	{
		...
		if (count($this->UserLevelPriv) == 0 && $this->isAdmin() && $Page != NULL && @$_SESSION[SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->terminate("master_user_roleslist.php");
		}
		return TRUE;
	}
	...
	public function userIDList()
	{
		...
		$len = count($ar);
		for ($i = 0; $i < $len; $i++)
			$ar[$i] = QuotedValue($ar[$i], DATATYPE_STRING, Config("USER_TABLE_DBID"));
		return implode(", ", $ar);
	}
	...
	function UserID_Loaded() {
		...
		// For phpMaker 2023
		// 2023 https://www.hkvforums.com/viewtopic.php?t=35925
		// 2023 $this->setSuccessMessage("Welcome, " . CurrentUserName()); // display welcome message to current user
		// 2023 	if (CurrentUserLevel() == "1") { // assume 1 = sales
		// 2023 		$this->Page_Terminate("sales.php");
		// 2023 	} elseif (CurrentUserLevel() == "2") { // assume 2 = finance
		// 2023 		$this->Page_Terminate("finance.php");
		// 2023 	} else { // default
		// 2023 	$this->Page_Terminate("index.php"); // default, redirect to index.php
		// 2023 	}
...
class UserProfile
{
	...
	public function __construct()
	{
		...
		// Password Expiry
		$this->Profile[Config("USER_PROFILE_LAST_PASSWORD_CHANGED_DATE")] = "";
	}
	...
	public function loadProfileFromDatabase($usr)
	{
		...
		$sql = $UserTable->getSql($filter);
		$rswrk = Conn($UserTable->Dbid)->execute($sql);
		if ($rswrk && !$rswrk->EOF) {
			$this->loadProfile(HtmlDecode($rswrk->fields(Config("USER_PROFILE_FIELD_NAME"))));
			$rswrk->close();
			return TRUE;
		}
		return FALSE;
	...
	public function passwordExpired($usr)
	{
		...
		$dt = @$this->Profile[Config("USER_PROFILE_LAST_PASSWORD_CHANGED_DATE")];
		if (strval($dt) == "")
			$dt = StdCurrentDate();
	...
	public function setPasswordExpired($usr)
	{
		...
		if ($this->loadProfileFromDatabase($usr)) {
			try {
				$this->Profile[Config("USER_PROFILE_LAST_PASSWORD_CHANGED_DATE")] = StdDate(strtotime('-' . ($this->PasswordExpiryTime + 1) . ' days'));
				$this->saveProfileToDatabase($usr);
				$this->Profile = $p; // Restore current profile
				return TRUE;
			} catch (\Exception $e) {
